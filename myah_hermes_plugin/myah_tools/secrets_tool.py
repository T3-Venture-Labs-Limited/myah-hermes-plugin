...
# ── Session-keyed secret request callbacks (mirrors skills_tool.py) ──
_secret_request_callbacks: Dict[str, object] = {}
_secret_request_session_key: contextvars.ContextVar[str] = contextvars.ContextVar(
    '_secret_request_session_key', default='',
)
...
def set_secrets_request_callback(session_key: str, callback) -> None:
    """Register (or unregister) a secret-request callback for a session."""
    if callback is None:
        _secret_request_callbacks.pop(session_key, None)
    else:
        _secret_request_callbacks[session_key] = callback


def set_secret_request_session_key(session_key: str) -> contextvars.Token:
    """Set the active session key (returns a reset token)."""
    return _secret_request_session_key.set(session_key)


def reset_secret_request_session_key(token: contextvars.Token) -> None:
    """Reset the session key to its prior value."""
    _secret_request_session_key.reset(token)
...
def _action_request(args: Dict[str, Any], **_kwargs) -> str:
    ...
    _cur_key = _secret_request_session_key.get()
    _callback = _secret_request_callbacks.get(_cur_key) if _cur_key else None
    ...
    try:
        result = _callback(key, prompt, metadata)
